...
三个页面区域使用相互独立的数据源。SharePoint 尚未配置时，Flow Status 和 Live Activity 会优先使用 Databricks LTAK 完成记录；Databricks 也不可用时才使用 Demo 回退。
...
- `Demo`：三块区域都使用本地演示数据。
- `Connected sources`：分别连接 Databricks、SharePoint Excel 和 SharePoint 日志目录；SharePoint 未配置时使用 Databricks LTAK 移动记录，其他连接失败时自动使用 Demo 回退。
...
当前 LTAK 回退视图只读取 `KQUIT = 'X'` 的完成记录，并按 `TANUM` 去重：`BWART = 261` 显示为 LB10 Movement Record，`BWART = 321` 显示为 LU04 Movement Record。Live Activity 使用 `BDATU + BZEIT` 作为建单日期时间；Flow Status 使用 `QDATU = 当天` 汇总各类型完成数量。
...
将动画命名为 `ai-employee.webp`（推荐，支持平滑半透明边缘）或 `ai-employee.gif`，并放入 `assets` 文件夹。页面会依次加载 WebP、GIF；均不存在时自动回退到 `assets/ai-operator.png`。
